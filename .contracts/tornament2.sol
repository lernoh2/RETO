...
contract GarettoTournament_V16 is IERC777Recipient, ReentrancyGuard {
    ...
    function _resolve() internal {
        ...
        for (uint8 step = 1; step <= 3; step++) {
            ...
            if (generator.queueSize() >= 2) {
                ...
                generator.requestBatch();
                randShuffle = uint256(keccak256(abi.encode(seed, step, "S")));
                randOutcome = uint256(keccak256(abi.encode(seed, step, "O")));
            }

            round = _nextRound(round, step, randShuffle, randOutcome);
